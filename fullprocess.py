...
logging.basicConfig(
    format="%(asctime)s - %(message)s", filename="fullprocess.log", level=logging.DEBUG
)
...
def model_drift_has_occurred(new_files: list):
    ...
    score = score_model(df)

    logging.info(f"f1 score on new data is {score}")

    if score < latest_score:
        logging.info(
            f"Model drift detected: Merged {new_files} have a lower score {score} than latest score {latest_score}"
        )
        return True
